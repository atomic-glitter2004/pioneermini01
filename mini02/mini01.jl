...
function main(filename = "graph09.txt")
    ...
    # Run label propagation to cluster nodes
    ...
    # Assign color indices based on final labels after clustering
    labels = unique([node.label for node in values(node_info)])
    sort!(labels)  # Optional: for consistent color assignment
